...
function openOverlay(action, id, currentId)
{
    ...
    .then(data => {
        data.users.forEach(d => {
            ...
            link.href = `/Instagram_Clone/accounts.php/${d.url}`;
            link.className = "flex flex-row justify-left gap-4";
            link.innerHTML = `
            ...
            layoutDiv.className = "flex justify-between items-center";
            layoutDiv.appendChild(link);
            ...
            if (d.id != currentId) {
                ...
                followBtn.className = `follow-btn px-4 py-2 font-medium tracking-wide text-white capitalize transition-colors duration-300 transform ${buttonClass} rounded-lg focus:outline-none focus:ring focus:ring-opacity-80`;
                followBtn.textContent = buttonText;

                layoutDiv.appendChild(followBtn);
            }

            newElement.appendChild(layoutDiv);
