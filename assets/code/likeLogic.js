...
<svg aria-label="Unlike" fill="currentColor" height="24" role="img" viewBox="0 0 48 48" width="24" class="text-red-500">
            ...
            fetch('/Instagram_Clone/src/api/like.php', {
                method: 'POST',
                body: formData,
            })
            ...
            .then(data => {
                if (data.likes !== undefined) {
                    ...
                    const iconContainer = button.querySelector('.like-icon');
                    if (iconContainer) {
                        iconContainer.innerHTML = liked ? likedIcon : unlikeIcon;
                    }
                    likeText.textContent = `${data.likes} likes`;
                }
            })
            .catch(error => {
