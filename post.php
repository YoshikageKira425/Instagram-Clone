...
if (!empty($_POST) && isset($_POST["action"])) {
    $postController->handleLikeAndSave($post["id"], $_POST["action"]);
    header("Location: /Instagram_Clone/post.php/" . $post["id"]);
    exit;
}
        ...
        <div class="flex flex-col">
            <div class="px-4 pt-5">
                <a href="/" class="text-white ">
                    <svg aria-label="Instagram" class="x1lliihq x1n2onr6 x5n08af" fill="currentColor" height="37.7" role="img" viewBox="32 4 113 32" width="133.9">
                        <title>Instagram</title>
        ...
        <div class="max-w-md text-white border border-gray-800 rounded-lg shadow mb-6">
            <div class="flex items-center justify-between px-4 py-3">
                <a href="/Instagram_Clone/accounts.php/<?= $postUser["url"] ?>" class="flex items-center space-x-3">
                    <img src="<?= $postUser["profile_image"] ?>" alt="User" class="w-10 h-10 rounded-full object-cover">
                    <span class="font-semibold text-sm"><?= $postUser["username"] ?></span>
                </a>
            </div>
            ...
            <div class="px-4 py-3">
                <form class="flex items-center space-x-4" method="post">
                    <?php if ($postController->isLiked($post["id"])): ?>
                        <button type="submit" name="action" value="unlike" class="text-red-500">
                            <svg aria-label="Unlike" fill="currentColor" height="24" role="img" viewBox="0 0 48 48" width="24">
                                <title>Unlike</title>
                ...
                <p class="mt-2 text-sm font-semibold"><?= $postController->GetLikesCount($post["id"]) ?> likes</p>

                <p class="mt-1 text-sm"><span class="font-semibold"><?= $postUser["username"] ?></span> <?= $post["content"] ?></p>
            </div>
        </div>
        <div class="w-80 bg-neutral-700 text-white rounded-lg shadow flex flex-col h-[500px] hidden" id="commentsSection">
            <div class="px-4 py-3 border-b border-gray-600 font-semibold">Comments</div>
            <div class="flex-grow overflow-y-auto px-4 py-3 space-y-3">
            ...
            <form class="px-4 py-3 border-t border-neutral-600 flex space-x-2" method="post">
                <input type="text" name="comment" placeholder="Add a comment..." class="flex-grow rounded bg-neutral-800 px-3 py-2 text-white placeholder-neutral-400 focus:outline-none" />
                <button type="submit" class="text-blue-400 font-semibold hover:text-blue-600">Post</button>
            </form>
        </div>
    </div>
